...
import MonitorSummary from "../../components/admin/ticket-monitor/MonitorSummary";
import MonitorFocus from "../../components/admin/ticket-monitor/MonitorFocus";

export default function AdminDashboard() {
  const { prices } = usePrice();
  useEffect(() => {
    apiAdmin
      .put("/api/admin/open-market")
  ...
  const switchOrderStatus = (status) => {
    ...
    apiAdmin
      .post("/apiadmin/order-switch", { pledgeIds: selectedTickets, status })
      .then(() => {
        fetchTicket();
  ...
  const fetchTicket = async () => {
    try {
      const response = await apiAdmin.get("/api/admin/monitor/ticket/all");
